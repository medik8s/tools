...
| Option | Description |
|--------|-------------|
| `--only LIST` | Remove only listed operators (`nhc`, `snr`, `nmo`, `mdr`, `far`, `sbr`). Default: all six. |
...
4. OperatorGroup `workload-availability-operator-group` (only when removing all six operators; skipped with `--only`)
